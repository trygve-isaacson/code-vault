...
/*
These are the Mac OS X-specific includes and definitions needed
...
//inline int putenv(char* env) { return ::putenv(env); } (not used in our Mac platform code)
//inline char* getenv(const char* name) { return ::getenv(name); } (not used in our Mac platform code)
